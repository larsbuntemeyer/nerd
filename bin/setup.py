...
def applyPreprocStack():
    #we are in object directory
    GVars.out.put('running experimental PreprocStack.py on sources in object'\
                  ' directory')
    cmd = os.path.join(GVars.nerdHomeDir, 'tools/scripts/PreprocStack.py')
    cmd = cmd + ' patch'
    os.system(cmd)
...
#ifndef NERD_DEFINES_INCLUDED
#define NERD_DEFINES_INCLUDED

#define NERD_NUMBER_OF_VARIABLES %(nvar)d
#define NERD_NUMBER_OF_FLUXES %(nflux)d
#define NERD_NUMBER_OF_GUARD_CELLS %(GUARDCELLS)d
#define NERD_NUMBER_OF_INT_PARTICLE_PROPS %(n_int_props)d
#define NERD_NUMBER_OF_REAL_PARTICLE_PROPS %(n_real_props)d
...
#\tprint the compiler flags to file in PREPROC/object/
...
def addTopModules(modules, sourceDir='.'):
    ...
    for module in topModules:
        if module not in modules:
            modules.append(PreprocModule(module, ignoreConfig=1))
            if os.path.isdir(os.path.join(module, NULLMODULE)):
                modules.append(PreprocModule(os.path.join(module, NULLMODULE),
                                           ignoreConfig=1))
    return modules
...
def readInputModulesFile(file, opts):
    ...
    modules = map(PreprocModule, opts.withModules) #redundant if used -auto
    ...
    for line in open(file).readlines():
        ...
        modules.append(PreprocModule(b))
        GVars.out.put(b)
        ...
        for module in modules[:]:
            for child in module['DEFAULT']:
                if child not in modules:
                    new = 1
                    modules.append(PreprocModule(child))
    return modules
    ...
    while new:
        ...
        for module in modules[:]:
            ...
            if (parent and (parent not in modules)):
                    new = 1
                    modules.append(PreprocModule(parent))
    return modules
    ...
    for module in modules:
        for reqs in module['REQUIRES']:
            ...
            if not good: #pick first module in an OR list
                ...
                out.append(PreprocModule(reqs[0]))
    return out
...
def generateModulesfile(outfile, opts):
    ...
    setup=PreprocModule(os.path.join('..', 'setups', opts.setupName))
    modules = [setup] + map(PreprocModule, opts.withModules)
...
class ModuleUnion(UserDict.UserDict):
    """Collates info from an ordered list of PreprocModule instances"""
    def __init__(self, modules):
        UserDict.UserDict.__init__(self)
        self['FLUX'] = []
        self['VARIABLE'] = {}
...
def runPreprocessor(flags=''):
    """Run the cpp nerdessor on all source files in current dir"""
...
def parseCommandLine(opts):
    USAGE="""usage:  setup <problem-name> [options]
        ...
        options: -auto -[123]d -maxblocks=<#> -nxb=<#> -nyb=<#> -nzb=<#>
                 ...
                 [-debug | -test] -nerdess -objdir=<relative obj directory>
...
"""
    ...
    for arg in args:
        ...
        elif arg == '-nerdess': opts.nerdess = 1
...
# consider removing
    ...
    if opts.nerdStack:
        opts.defines.append('-DNERD_STACK')
...
def main():
    ...
    # the [:-4] was added here because setup.py now resides in NERD_HOME/bin
    ...
    setupModule = PreprocModule(os.path.join('..', 'setups', opts.setupName))
